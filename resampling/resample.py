...
def sample_near_dkoi(
    ...
    *,
    ...
    (kepmag, rad, logp) distribution matches that of dkoi.
    ...
    dkoi : pandas.DataFrame
        Target catalog whose (kepmag, rad, logp) distribution is matched
    ...
    rad_col : str
        Column for stellar radius (or other matching variable).
    logp_col : str
        Column for logp (or other third matching variable).
    x_col : str
        Column for Teff.
    k : int
        Number of nearest neighbors in 3D (kepmag, rad, logp) space
    ...
    bw_logp : float
        Bandwidth for logp distance scaling.
    ...
    """
    need_cols = [m_col, rad_col, logp_col, x_col]
    ...
    for b, n_draw in zip(bins, draw_counts):
        ...
        for j in range(len(tgt_sel)):
            ...
            # dd is Euclidean distance in the scaled 3D space:
            # dd^2 = (Δm/bw_m)^2 + (Δrad/bw_rad)^2 + (Δlogp/bw_logp)^2
